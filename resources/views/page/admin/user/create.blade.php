...
<section class="content">
    <div class="border-1 card rounded-2">
        <div class="card-body bg-white rounded-2">
            <form id="form-kegiatan" method="POST" action="{{ route('admin.akun.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('role') ? 'is-invalid' : '' }}" name="role" id="field-role" data-placeholder="Pilih Hak Akses">
                                <option></option>
                                <option value="superadmin" {{ old('role') == 'superadmin' ? 'selected="selected"' : '' }}>Super Admin</option>
                                <option value="admin" {{ old('role') == 'admin' ? 'selected="selected"' : '' }}>Admin</option>
                                <option value="kecamatan" {{ old('role') == 'kecamatan' ? 'selected="selected"' : '' }}>Kecamatan</option>
                                <option value="kelurahan" {{ old('role') == 'kelurahan' ? 'selected="selected"' : '' }}>Kelurahan</option>
                                <option value="lkk" {{ old('role') == 'lkk' ? 'selected="selected"' : '' }}>LKK</option>
                            </select>
